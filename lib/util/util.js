 ...
 * @param {number} depth Глубина слияния.
 */
util.merge = function(base, target, depth) {
  if(undefined === depth) {
    depth = 0;
  }
  for (var key in target) {
    if(base[key] instanceof Object && target[key] instanceof Object && depth !== 0) {
      base[key] = util.merge(base[key], target[key], depth - 1);
    } else {
      base[key] = target[key];
    }
  }
