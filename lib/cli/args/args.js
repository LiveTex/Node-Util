...
cli.args.extractValues = function(agrv) {
  var args = agrv.concat('');

  var currentArg = '';
  while (args.length > 0) {
    var item = args.shift().replace(/=/g, ' ');

    var spaceIndex = item.indexOf(' ');
    if (spaceIndex !== -1) {
      currentArg =
          cli.args.__applyArgument(item.substring(0, spaceIndex), currentArg);
      currentArg =
          cli.args.__applyArgument(item.substr(spaceIndex + 1), currentArg);
    }
  }
};
